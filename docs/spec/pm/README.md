# `docs/spec/pm/` — package-manager behavior specs
...
> Family: foundational reference — git-tracked and published alongside [`docs/spec/formats/`](../formats) and [`docs/spec/registry/`](../registry).
...
6. **Lockfile + registry** — what it persists (→ [`docs/spec/formats/`](../formats)) + how it fetches (→ [`docs/spec/registry/`](../registry)).
...
That split — who keeps Node's resolver vs who supplants it — is what `docs/spec/pm/`
...
How each PM checks that fetched bytes match the lock. Details + citations in each per-PM doc; the shared SRI substrate is [`docs/spec/formats/_common.md` §3](../formats/_common.md#3-integrity-model).
...
- **Lockfile encodings** live in [`docs/spec/formats/`](../formats) — these docs cite them for *what* is persisted, they don't re-document the byte grammar.
- **Registry wire contracts** live in [`docs/spec/registry/`](../registry) — cited for *how* artefacts are fetched.
...
[`docs/spec/formats/`](../formats) and [`docs/spec/registry/`](../registry) (the rest of the
`docs/spec/` tree stays contributor-private). They were composed from
